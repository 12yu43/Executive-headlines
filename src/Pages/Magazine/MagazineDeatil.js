...
export default function MagazineDeatil() {
    ...
    const getData = useCallback(async () => {
        setLoader(true);
        try {
            const resp = await FetchApi(`${Endpoints.GetNewsDetail}/magazine/${param?.slug}`);
                                ...
                                <div className="col-12">
                                    <div className="post-header" style={{ backgroundImage: "url(../img/bg/page-banner-fashion.jpg)" }}>
                                        ...
                                        <div className="meta fix">
                                            <a href=" " className="meta-item category fashion" aria-label="Cover Story">Cover Story</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="container mb-50">
                                ...
                                <div className="row">
                                    <div className="col-md-4">
                                        <Link to={`/cover-story/${CoverData?.url}`}>
                                            <img src={`${Endpoints.ImageUrl}${data?.magazine_cover_image}`} alt="Cover Story" className="w-100" style={{ border: "5px solid red" }} />
                                        </Link>
                                    </div>
                        ...
                        <div className="text-center mt-5">
                            <Link to={`/listing/${data?.url}`}>
                                <img src={`${Endpoints.ImageUrl}${data?.issue_logo}`} alt="Issue Logo" className="w-50 border p-3" />
                            </Link>
                        </div>
                                    ...
                                    <div className="card">
                                        <div className="card-body">
                                            <Link to={`/feature/${item?.url}`}>
                                                <img src={`${Endpoints.ImageUrl}${item?.images}`} alt={item?.title} className="w-100" />
                                            </Link>
